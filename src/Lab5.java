...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener {
	...
	public void stworzKorkociag(GL2 gl2, int radius, int numbersOfTurns) {
		gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);

		int numVertices = numbersOfTurns * 1000;
		double angelIncrement = numbersOfTurns * Math.PI / numVertices;
		float height = -8;

		gl2.glBegin(GL2.GL_POLYGON);
		for (int i = 0; i <= numVertices; i++) {
			gl2.glColor3f(x / 0.6f, (x + z) * x / 0.4f, numbersOfTurns / 0.5f);
			x = (float) (radius * Math.cos(angel));
			z = (float) (radius * Math.sin(angel));
	...
	public void Polygon(GL2 gl2, int numVertices) {
		...
		gl2.glBegin(GL2.GL_POLYGON);
		double angelIncrement = 2 * Math.PI / numVertices;
		for (int i = 0; i <= numVertices; i++) {
	...
	public void stworzPiramide(GL2 gl2, double size, int n) {
		...
		} // Stworzenie Wielok¹ta

		for (int i = 0; i < X.length; i++) {
			gl2.glBegin(GL2.GL_POLYGON);
			gl2.glColor3f(1, 0, 0); // Nadanie koloru jednemu punktu
